    ...
    for (int head = 0; head < heads; ++head) {
        for (int bs = 0; bs < batchSize; ++bs) {
            for (int seq = 0; seq < seqLen; ++seq) {
                ...
                float16_t *q = query + bs * seqLen * qStride + seq * qStride + head * dim;
                float16_t *k = key + bs * seqLen * kStride + seq * kStride + head * dim;
...
void QwenRotaryEmbedding::forward(float16_t *query, float16_t *key, int totSeqLen, int qStride, int kStride,
        int qHeads, int kHeads, int *positionIds) {
    printf("Unsupported QwenRotaryEmbedding in cb mode !\n");
    exit(1);
}