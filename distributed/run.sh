...
FIRST_TOKEN_WEIGHT_LOCATION=$1 NEXT_TOKEN_WEIGHT_LOCATION=$2 OMP_NUM_THREADS=$3 \
	taskset -c $cpu_index \
	numactl -C $cpu_index -m $2 $BENCHMARK -m $model_path -t $model_token_path -d $data_type -l $input_length --loop $loop_count --output_len $output_length -b $batch_size --no_stream