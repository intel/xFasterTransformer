...
// with open("./gemm_kernel_ext.cpp", "r") as in_file, open("./gemm_kernel_ext_fp16.cpp", "w") as out_file:
//     # Loop over each line in the input file
//     for line in in_file:
//         if "float *B" in line:
//             line = line.replace("float *B", "float16_t *B")
//         if "const float *pB" in line:
//             line = line.replace("const float *pB", "const float16_t *pB")
//         # Change the method to load from B
//         if "_mm512_loadu_ps(pB)" in line:
//             line = line.replace("_mm512_loadu_ps(pB)", "_mm512_cvtph_ps(_mm256_maskz_loadu_epi16(0xFFFF, pB))")
//         if "_mm512_maskz_loadu_ps(mask, pB)" in line:
//             line = line.replace("_mm512_maskz_loadu_ps(mask, pB)", "_mm512_cvtph_ps(_mm256_maskz_loadu_epi16(mask, pB))")
...
void small_gemm_transb(const float *A, const float16_t *B, float *C, int K, int lda, int ldb, int ldc) {
    // vc[0] vc[1]   ... vc[N-1]
    // vc[N] vc[N+1] ...
    // ..
    // vc[(M-1)*N] ...
    ...
    for (int v = 0; v < vecs - 1; ++v) {
        ...
        compile_time_for<M * N>::op([&](auto i) {
            ...
            if constexpr (idx < N) {
                vb[idx] = _mm512_cvtph_ps(_mm256_maskz_loadu_epi16(0xFFFF, pB));
                pB += ldb;
            }
    ...
    // The last vector computing, together with data store
    {
        ...
        const float16_t *pB = B + (vecs - 1) * 16;
        float *pC = C;
        ...
        compile_time_for<M * N>::op([&](auto i) {
            ...
            if constexpr (idx < N) {
                vb[idx] = _mm512_cvtph_ps(_mm256_maskz_loadu_epi16(mask, pB));
                pB += ldb;
            }
            constexpr int col = idx % N;
            vc[idx] = _mm512_fmadd_ps(va, vb[col], vc[idx]);
...
void small_gemm_transb_6x4(const float *A, const float16_t *B, float *C, int K, int lda, int ldb, int ldc) {
    ...
    for (int v = 0; v < vecs - 1; ++v) {
        ...
        pA += lda;
        vb[0] = _mm512_cvtph_ps(_mm256_maskz_loadu_epi16(0xFFFF, pB));
        pB += ldb;
        vc[0] = _mm512_fmadd_ps(va, vb[0], vc[0]);
        vb[1] = _mm512_cvtph_ps(_mm256_maskz_loadu_epi16(0xFFFF, pB));
        pB += ldb;
        vc[1] = _mm512_fmadd_ps(va, vb[1], vc[1]);
        vb[2] = _mm512_cvtph_ps(_mm256_maskz_loadu_epi16(0xFFFF, pB));
        pB += ldb;
        vc[2] = _mm512_fmadd_ps(va, vb[2], vc[2]);
        vb[3] = _mm512_cvtph_ps(_mm256_maskz_loadu_epi16(0xFFFF, pB));
        pB += ldb;
    ...
    pA += lda;
    vb[0] = _mm512_cvtph_ps(_mm256_maskz_loadu_epi16(mask, pB));
    pB += ldb;
    vc[0] = _mm512_fmadd_ps(va, vb[0], vc[0]);
    pC[0] = _mm512_reduce_add_ps(vc[0]);
    vb[1] = _mm512_cvtph_ps(_mm256_maskz_loadu_epi16(mask, pB));
    pB += ldb;
    vc[1] = _mm512_fmadd_ps(va, vb[1], vc[1]);
    pC[1] = _mm512_reduce_add_ps(vc[1]);
    vb[2] = _mm512_cvtph_ps(_mm256_maskz_loadu_epi16(mask, pB));
    pB += ldb;
    vc[2] = _mm512_fmadd_ps(va, vb[2], vc[2]);
    pC[2] = _mm512_reduce_add_ps(vc[2]);
    vb[3] = _mm512_cvtph_ps(_mm256_maskz_loadu_epi16(mask, pB));
    pB += ldb;
    vc[3] = _mm512_fmadd_ps(va, vb[3], vc[3]);
...
template <int M>
void small_gemm_transb(const float *A, const float16_t *B, float *C, int N, int K, int lda, int ldb, int ldc) {
    int j = 0;
    const float *pA = A;
    constexpr int NB = 4;
