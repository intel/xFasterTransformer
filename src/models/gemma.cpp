...
GemmaLLM<WeiT, KVCacheT>::GemmaLLM(const std::string &modelPath)
    ...
    // Embedding
    embedding = new GemmaTokenEmbedding<float16_t>(ctx);
...
template <typename WeiT, typename KVCacheT>
void GemmaLLM<WeiT, KVCacheT>::embeddingForward(int *ids, float *output, int tokenSize) {
    embedding->forward(ids, output, tokenSize);
}

template <typename WeiT, typename KVCacheT>
void GemmaLLM<WeiT, KVCacheT>::embeddingForward(int *ids, bfloat16_t *output, int tokenSize) {
    embedding->forward(ids, output, tokenSize);
}
