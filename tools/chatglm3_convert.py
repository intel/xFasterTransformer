"""
Convert huggingface ChatGLM model. Use https://huggingface.co/THUDM/chatglm3-6b
"""
...
def split_and_convert(args):
    ...
    try:
        config["chatglm3"]["model_name"] = (
            "chatglm3" if hf_config["_name_or_path"] == "" else hf_config["_name_or_path"]
        )
        num_attention_heads = config["chatglm3"]["head_num"] = str(hf_config["num_attention_heads"])
        n_embd = hf_config["hidden_size"]
        config["chatglm3"]["size_per_head"] = str(n_embd // hf_config["num_attention_heads"])
        config["chatglm3"]["inter_size"] = str(hf_config["ffn_hidden_size"])
        config["chatglm3"]["max_pos_seq_len"] = str(hf_config["seq_length"])
        config["chatglm3"]["num_layer"] = str(hf_config["num_layers"])
        config["chatglm3"]["layernorm_eps"] = str(hf_config["layernorm_epsilon"])  # "1e-5"
        config["chatglm3"]["layernorm_type"] = "pre_layernorm"
        config["chatglm3"]["activation_type"] = "swiglu"
        config["chatglm3"]["has_post_decoder_layernorm"] = "1" if str(hf_config["post_layer_norm"]) == "True" else "0"
        config["chatglm3"]["vocab_size"] = str(hf_config["padded_vocab_size"])
        config["chatglm3"]["start_id"] = str(hf_config["bos_token_id"])
        config["chatglm3"]["end_id"] = str(hf_config["eos_token_id"])
        config["chatglm3"]["weight_data_type"] = args.weight_data_type

        kv_channels = config["chatglm3"]["kv_channels"] = str(hf_config["kv_channels"])
        config["chatglm3"]["rmsnorm"] = "1" if str(hf_config["rmsnorm"]) == "True" else "0"
        config["chatglm3"]["apply_residual_connection_post_layernorm"] = (
            "1" if str(hf_config["apply_residual_connection_post_layernorm"]) == "True" else "0"
        )
        config["chatglm3"]["multi_query_attention"] = "1" if str(hf_config["multi_query_attention"]) == "True" else "0"
        multi_query_group_num = config["chatglm3"]["kv_head_num"] = str(hf_config["multi_query_group_num"])
        config["chatglm3"]["pad_id"] = str(hf_config["pad_token_id"])
