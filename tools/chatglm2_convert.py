...
def split_and_convert_process(i, saved_dir, factor, key, args, val, old_name, dtype, num_attention_heads, multi_query_group_num, kv_channels):
...
def split_and_convert(args):
    ...
    try: 
        config["chatglm2"]["model_name"] = "chatglm2" if hf_config["_name_or_path"] == '' else hf_config["_name_or_path"]
        ...
        config["chatglm2"]["apply_residual_connection_post_layernorm"] = '1' if str(hf_config["apply_residual_connection_post_layernorm"])=='True' else "0"
        ...
        if name == 'transformer.embedding.word_embeddings.weight':
            ...
            for i in range(len(huggingface_model_name_pattern)):
                if huggingface_model_name_pattern[i] in name:
                    factor = 1
                    new_name = name.replace("transformer.encoder.layers.", "layers.").replace(huggingface_model_name_pattern[i], ft_model_name_pattern[i])
                    starmap_args.append((0, saved_dir, factor, new_name, args,
                                        param.detach().cpu().numpy().astype(np_weight_data_type),
                                        name, np_weight_data_type, num_attention_heads, multi_query_group_num, kv_channels))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-in_file', '-i', type=str, default='/data/chatglm-6b', help='file name of input checkpoint file')
