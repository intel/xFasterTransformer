...
MixtralLLM<WeiT>::MixtralLLM(const std::string &modelPath)
    : CommonDecoder<Attention<WeiT, LlamaRotaryEmbedding, RmsNorm, typename TypeSelector<WeiT>::InType,
            ...
            MoeMLP<WeiT, typename TypeSelector<WeiT>::InType, typename TypeSelector<WeiT>::ImType,
                    typename TypeSelector<WeiT>::OutType>,
            typename TypeSelector<WeiT>::KVCacheType>(modelPath, "Mixtral") {
    // Context
    ...
    // Moe Block
    setMoeBlockWeight(modelPath);
}

template <typename WeiT>
MixtralLLM<WeiT>::~MixtralLLM() {
    delete embedding;
}

template <typename WeiT>
void MixtralLLM<WeiT>::setEmbeddingWeights(const std::string &modelPath) {
...
template <typename WeiT>
void MixtralLLM<WeiT>::setFinalLnWeight(const std::string &modelPath) {
    int hiddenSize = embedding->getHiddenSize();
...
template <typename WeiT>
void MixtralLLM<WeiT>::setMoeBlockWeight(const std::string &modelPath) {}
...
template <typename WeiT>
void MixtralLLM<WeiT>::prepareAttnMask(int *ids, int step) {
    DecoderContext *ctx = this->getContext();
...
template <typename WeiT>
void MixtralLLM<WeiT>::embeddingForward(int *ids, float *output, int batchSize, int seqLen) {
    embedding->forward(ids, output, batchSize, seqLen);
}

template <typename WeiT>
void MixtralLLM<WeiT>::embeddingForward(int *ids, bfloat16_t *output, int batchSize, int seqLen) {
    embedding->forward(ids, output, batchSize, seqLen);
}

template <typename WeiT>
void MixtralLLM<WeiT>::lastLayerNormForward(float *input, float *output, int rows) {
    finalLN.forward(input, output, rows);
}

template <typename WeiT>
void MixtralLLM<WeiT>::lastLayerNormForward(bfloat16_t *input, bfloat16_t *output, int rows) {
    finalLN.forward(input, output, rows);
}

template class MixtralLLM<float>;
template class MixtralLLM<float16_t>;
template class MixtralLLM<bfloat16_t>;
template class MixtralLLM<int8_t>;
template class MixtralLLM<w8a8_t>;
template class MixtralLLM<uint4x2_t>;
template class MixtralLLM<nf4x2_t>;