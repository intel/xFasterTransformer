...
template <typename WeiT>
GptNeoDecoder<WeiT>::~GptNeoDecoder() {
    delete embedding;
}

template <typename WeiT>
void GptNeoDecoder<WeiT>::setEmbeddingWeights(const std::string &modelPath) {
...
template <typename WeiT>
void GptNeoDecoder<WeiT>::prepareAttnMask(int *ids, int step) {
    DecoderContext *ctx = this->getContext();
...
template <typename WeiT>
void GptNeoDecoder<WeiT>::embeddingForward(int *ids, float *buf, int batchSize, int seqLen) {
...
template <typename WeiT>
void GptNeoDecoder<WeiT>::lastLayerNormForward(float *input, float *output, int rows) {
    finalLN.forward(input, output, rows);
}

template class GptNeoDecoder<float>;
template class GptNeoDecoder<float16_t>;
template class GptNeoDecoder<bfloat16_t>;
template class GptNeoDecoder<int8_t>;
template class GptNeoDecoder<w8a8_t>;
template class GptNeoDecoder<uint4x2_t>;
template class GptNeoDecoder<nf4x2_t>;
