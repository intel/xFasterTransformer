...
TeleChatLLM<WeiT, KVCacheT>::TeleChatLLM(const std::string &modelPath)
    : CommonDecoder<Attention<WeiT, TeleChatRotaryEmbedding, RmsNorm, typename TypeSelector<WeiT>::InType,
...
template <typename WeiT, typename KVCacheT>
TeleChatLLM<WeiT, KVCacheT>::~TeleChatLLM() {
    delete embedding;
    delete finalLN;
}

template <typename WeiT, typename KVCacheT>
void TeleChatLLM<WeiT, KVCacheT>::setEmbeddingWeights(const std::string &modelPath) {
    embedding->setWeights(modelPath + "/model.wte.bin");
}

template <typename WeiT, typename KVCacheT>
void TeleChatLLM<WeiT, KVCacheT>::setFinalLnWeight(const std::string &modelPath) {
...
template <typename WeiT, typename KVCacheT>
void TeleChatLLM<WeiT, KVCacheT>::prepareAttnMask(int *ids, int step) {
    DecoderContext *ctx = this->getContext();
...
template <typename WeiT, typename KVCacheT>
void TeleChatLLM<WeiT, KVCacheT>::embeddingForward(int *ids, float *output, int tokenSize) {
    embedding->forward(ids, output, tokenSize);
}

template <typename WeiT, typename KVCacheT>
void TeleChatLLM<WeiT, KVCacheT>::embeddingForward(int *ids, bfloat16_t *output, int tokenSize) {
    embedding->forward(ids, output, tokenSize);
}

template <typename WeiT, typename KVCacheT>
void TeleChatLLM<WeiT, KVCacheT>::embeddingForward(int *ids, float16_t *output, int tokenSize) {
    embedding->forward(ids, output, tokenSize);
}

template <typename WeiT, typename KVCacheT>
void TeleChatLLM<WeiT, KVCacheT>::lastLayerNormForward(float *input, float *output, int rows) {
    finalLN->forward(input, output, rows);
}

template <typename WeiT, typename KVCacheT>
void TeleChatLLM<WeiT, KVCacheT>::lastLayerNormForward(bfloat16_t *input, bfloat16_t *output, int rows) {
    finalLN->forward(input, output, rows);
}

template <typename WeiT, typename KVCacheT>
void TeleChatLLM<WeiT, KVCacheT>::lastLayerNormForward(float16_t *input, float16_t *output, int rows) {
    finalLN->forward(input, output, rows);
}

IMPLEMENT_MODEL(TeleChatLLM, telechat)