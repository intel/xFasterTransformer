...
class MixtralConvert(BaseModelConvert):
    """
    Convert huggingface Mixtral model. Support both Mixtral & Mixtral2.
    """
    ...
    def split_and_convert_process(self, i, output_dir, factor, key, val, num_attention_heads, num_key_value_heads):
        ...
        elif "experts.gate.weight" in key:
    ...
    def split_and_convert(self, input_dir, output_dir, dtype, processes):
        ...
        print("\n")
        print(f"[INFO] model loaded {model}")
        print("\n")
        print(f"[INFO] model config loaded {hf_config}")
        print("\n")
        ...
        try:
            ...
            config["Mixtral"]["num_experts_per_tok"] = str(hf_config["num_experts_per_tok"])
            config["Mixtral"]["num_local_experts"] = str(hf_config["num_local_experts"])
        ...
        def _return_export_name(i):
            return {
                f"block_sparse_moe.experts.{i}.w1.weight": f"experts.{i}.mlp.gate_proj.weight",
                f"block_sparse_moe.experts.{i}.w2.weight": f"experts.{i}.mlp.down_proj.weight",
                f"block_sparse_moe.experts.{i}.w3.weight": f"experts.{i}.mlp.up_proj.weight",
            }
        ...
        ] + list(chain(*[_return_export_name(i).keys() for i in range(hf_config["num_local_experts"])]))
        ...
        ft_model_name_pattern = [
            ...
            "mlp.down_proj.weight",
            "experts.gate.weight",
        ] + list(chain(*[_return_export_name(i).values() for i in range(hf_config["num_local_experts"])]))
