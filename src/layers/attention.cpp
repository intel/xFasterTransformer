...
void invokeAttentionLLaMA(DataType dt, int batchSize, int inputSeqLen, int attHeadSize, int attHeadNum, int kvHeadNum,
    ...
    if (dt == DataType::bf16) {
        ...
        if (ctx == nullptr || (ctx != nullptr && (ctx->hiddenSize != hiddenSize || ctx->attHeadSize != attHeadSize))) {
            if (ctx != nullptr) delete ctx;
            printf(">> create context: %d %d\n", hiddenSize, attHeadSize);
            ctx = new DecoderContext(1, hiddenSize, attHeadSize, attHeadNum, kvHeadNum, 1, "silu", 1e-6, 0, 0,
