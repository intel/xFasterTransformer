...
AutoModel::AutoModel(std::string modelPath, xft::DataType datatype) : Model() {
    ...
    } else if (modeltype == "chatglm2_3") {
        switch (datatype) {
            case xft::DataType::fp16: setDecoder(new ChatGLM2_3<float16_t, RmsNorm>(modelPath)); break;
            case xft::DataType::bf16: setDecoder(new ChatGLM2_3<bfloat16_t, RmsNorm>(modelPath)); break;
            case xft::DataType::int8: setDecoder(new ChatGLM2_3<int8_t, RmsNorm>(modelPath)); break;
            case xft::DataType::bf16_fp16:
                setDecoder(new HybridModel<ChatGLM2_3, bfloat16_t, float16_t>(modelPath));
                break;
            case xft::DataType::bf16_int8: setDecoder(new HybridModel<ChatGLM2_3, bfloat16_t, int8_t>(modelPath)); break;
            default: printf("Unsupported data type.\n"); exit(-1);
        }
    } else {
