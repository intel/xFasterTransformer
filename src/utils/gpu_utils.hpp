...
inline int parse_engine_index(
        int argc, char **argv, int extra_args = 0) {
    // Returns default engine kind, i.e. CPU, if none given
    if (argc == 1) {
        return 0;
    } else if (argc <= extra_args + 3) {
        int engine_index = atoi(argv[2]);
        if (engine_index >= 0) {
            return engine_index;
        }
    }
    ...
    // If all above fails, the example should be ran properly
    std::cout << "Inappropriate engine index." << std::endl
              << "Please run the example like this: " << argv[0] << " [cpu|gpu] [0/1/2...]"
              << (extra_args ? " [extra arguments]" : "") << "." << std::endl;
    exit(1);
}
